...
class BannerAddressModel extends Model
{
     ...
     * @return string
     */
    public function bannerAddressTree($selectId = 0, $currentCid = 0)
    {
        $where = ['delete_time' => 0];
        if (!empty($currentCid)) {
     ...
     * @return string
     */
    public function bannerAddressTableTree($currentIds = 0, $tpl = '')
    {
        $where = ['delete_time' => 0];
//        if (!empty($currentCid)) {
//            $where['id'] = ['neq', $currentCid];
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']     = cmf_url('Banner/List/index', ['id' => $item['id']]);;
            $item['str_action'] = '  <a href="' . url("BannerAddress/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("BannerAddress/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            array_push($newCategories, $item);
        }
